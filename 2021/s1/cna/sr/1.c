...
static int ACKed[WINDOWSIZE];         /*array to store pck that recived ask*/
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet))
  {
    ...
    expectedACK = A_nextseqnum - windowcount;
    if (expectedACK < 0)
    {
    ...
    /* check if new ACK or duplicate*/
    if ((windowcount > 0) && (seqcount < WINDOWSIZE))
    {
      ...
      ackcount = (seqcount + windowfirst) % WINDOWSIZE;
...
void A_timerinterrupt(void)
{
  ...
  /*  resend the oldest packet in window*/
