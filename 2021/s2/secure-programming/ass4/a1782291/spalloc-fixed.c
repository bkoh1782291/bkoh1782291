...
// 4030 should be PAGESIZE
void *large_alloc(uint64_t size) {
  if (size > ~0ULL-3*PAGESIZE)
    return NULL;
  uint64_t pages = (size + PAGESIZE)>>PAGESHIFT; // bug 2 fix 4096 -> PAGESIZE
  void * rv = mmap(NULL,
      			(pages + 2) << PAGESHIFT,
			PROT_READ|PROT_WRITE,
  ...
  if (rv == MAP_FAILED)
    return NULL;
  *((uint64_t *)rv) = pages;
  mprotect(rv, PAGESIZE, PROT_READ); // bug 2 fix 4096 -> PAGESIZE
